def create_product(client, quantity=10):
    response = client.post("/products/", json={
        "name": "Armário", "category": "Cozinha", "price": 1200, "quantity": quantity
    })
...
def test_webhook_updates_existing_product(client):
    ...
    response = client.post("/webhooks/stock", json={
...
def test_list_sync_logs(client):
    ...
    client.post("/webhooks/stock", json={
        "product_id": product_id, "new_quantity": 5, "source": "erp-odoo"
    })
    client.post("/webhooks/stock", json={
        "product_id": 9999, "new_quantity": 5, "source": "erp-odoo"
    })
